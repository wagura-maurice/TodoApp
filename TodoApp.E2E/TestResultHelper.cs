...
namespace TodoApp.E2E
{
    ...
    {
        private static readonly string BaseReportsPath = Path.Combine(Directory.GetCurrentDirectory(), "TestReports");
        ...
        public static async Task SaveMarkdownReportAsync(string markdownContent)
        {
            try
            {
                var markdownPath = GetMarkdownPath();
                var fileName = "test_report.md";
            ...
            {
                TestContext.Out.WriteLine($"Failed to save markdown report: {ex.Message}");
            }
        }
    }
}
